...
# Renombra las columnas según la segunda imagen proporcionada
...
## 2.2 CORRELATIONS on differentiated series
...
### 2.2.1 Multicollinearity on differentiated series
...
## SPLIT (tried but finally not done as explained in the report)
...
also here not possible to use lag=2
...
### 2.2.3 Impulse-Response Functions and FEVD
...
```{python}
# needed to be implemented by hand to change the colors
import pandas as pd
import matplotlib.pyplot as plt
import numpy as np
...
# the actual data is in the last column of train_matrix
actual_values <- train_matrix[, 12]  

# MSE using the Metrics package
mse_value <- mse(actual_values, fitted_values[, 12])  
...
# Loop for cross validation rolling window 
...
# new date range starting from 1992
new_index = pd.date_range(start='1992', periods=len(df), freq='Y')
df.index = new_index


# fitted values (predictions from the VAR model)
fitted_values = var_results.fittedvalues

# actual values for 'remittances', last column
...
additional plot in original scale

```{python}

new_index = pd.date_range(start='1990', periods=len(data), freq='Y')

# new date range as the index 
...
#print(f"R² of the VAR model for remittances on the integrated series is: {r2_value_int}")
#print(f"R² of the VAR model for remittances on the differenced series is: {r2_value_diff}")
...
not moving window, first iteration 25 años, second iteration 26 años.... Finally not used
...
printing the values will be converted to the original scale:
```{python}
import pandas as pd
import numpy as np
from statsmodels.tsa.api import VAR
...
predictions_to_convert = df.iloc[:window_size, 11].tolist()  
actual_values = []
...
# date range starting from 1990 to match the original series
start_year = 1990
...
# "remittence" column from future predictions
...
plt.legend()
plt.grid(True)  # Añadir esta línea para mostrar la cuadrícula
plt.show()
